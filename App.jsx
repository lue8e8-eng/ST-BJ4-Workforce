              ...
              <div className="space-y-3">
                {personMonthlyStats.map(s => (
                  <div key={s.name} className="p-3 bg-white/5 rounded-xl border border-white/10">
                    <div className="flex justify-between font-bold mb-1"><span className="text-lg">{s.name}</span><span className="text-blue-400 font-bold text-xl" style={{ fontVariantNumeric: 'tabular-nums' }}>{s.work}</span></div>
                ...
                {records.map(r => {
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      <td className="p-5 text-slate-600" style={{ fontVariantNumeric: 'tabular-nums' }}>{r.date}</td>
                      ...
                      <td className="p-5 text-blue-500 font-bold" style={{ fontVariantNumeric: 'tabular-nums' }}>{isEditing ? <input className="w-24 border p-1 rounded text-center bg-blue-50 outline-none" value={editBuffer.startTime} onChange={e=>setEditBuffer({...editBuffer, startTime:e.target.value})}/> : r.startTime}</td>
                      <td className="p-5 text-blue-500 font-bold" style={{ fontVariantNumeric: 'tabular-nums' }}>{isEditing ? <input className="w-24 border p-1 rounded text-center bg-blue-50 outline-none" value={editBuffer.endTime} onChange={e=>setEditBuffer({...editBuffer, endTime:e.target.value})}/> : r.endTime}</td>
                      <td className="p-5 text-slate-400 text-xs" style={{ fontVariantNumeric: 'tabular-nums' }}>
                        {isEditing ? 
                      ...
                      <td className="p-5 text-blue-600 font-black text-base" style={{ fontVariantNumeric: 'tabular-nums' }}>{secondsToTime(stats.workSec)}</td>
                      <td className="p-5">
                        {isEditing ? 
