      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-9 bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-6 text-center">
                        {isEditing ? 
                          <div className="flex justify-center gap-2"><button onClick={()=>{setRecords(records.map(x=>x.id===r.id?editBuffer:x)); setEditingId(null);}} className="text-emerald-600 p-1 hover:bg-emerald-50 rounded-full"><Check size={18}/></button><button onClick={()=>setEditingId(null)} className="text-slate-400 p-1 hover:bg-slate-100 rounded-full"><X size={18}/></button></div> :
                          <div className="flex justify-center gap-2"><button onClick={()=>{setEditingId(r.id); setEditBuffer({...r});}} className="text-blue-400 p-1 hover:bg-blue-50 rounded-full"><Edit2 size={16}/></button><button onClick={()=>{if(window.confirm('確定刪除？')) setRecords(records.filter(x=>x.id!==r.id))}} className="text-slate-200 hover:text-rose-600 p-1 rounded-full"><Trash2 size={16}/></button></div>
                        }
                      </td>
                    </tr>
                  )
            ...
            {records.length === 0 && <div className="p-20 text-center text-slate-400 italic font-medium">尚未有資料</div>}
          </div>
        </div>
      </div>
    </div>
